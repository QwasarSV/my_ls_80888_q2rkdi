...
bool time_cmp(file_list* curr_node, file_list* next_node){
    if(curr_node->sb->st_mtim.tv_sec == next_node->sb->st_mtim.tv_sec){
        if(curr_node->sb->st_mtim.tv_sec == next_node->sb->st_mtim.tv_nsec){
            return lex_cmp(curr_node, next_node);
        }
        return curr_node->sb->st_mtim.tv_nsec < next_node->sb->st_mtim.tv_nsec;
    }
    return curr_node->sb->st_mtim.tv_sec < next_node->sb->st_mtim.tv_sec;
}
